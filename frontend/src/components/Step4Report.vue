...
const toggleRawResult = (timestamp, event) => {
  // Save button's position relative to the viewport
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response received on this platform)' || t === '[No reply]'
    }
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 (## xxx), since section title is already displayed in outer layer
...
const extractFinalContent = (response) => {
  ...
  // Legacy fallback for "Final Answer" in alternate format
