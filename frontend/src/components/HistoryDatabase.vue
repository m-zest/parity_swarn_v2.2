...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count for the current row to center each row
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Use debounce to delay state toggle, preventing rapid flickering
...
.grid-pattern {
  ...
  /* Position from top-left; height changes expand at bottom only, preserving grid position */
