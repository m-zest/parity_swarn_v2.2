...
4. Support user dialogue with autonomous retrieval tool invocation
"""
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        """Set up file handler to write logs to file as well"""
        import logging
...
This is not LLM simulation, but calls the real interview interface to get the original responses from simulation Agents.
...
Review this future rehearsal from a 'God's perspective':
...
1. [Must call tools to observe the simulation world]
   - You are observing the future's rehearsal from a 'God's perspective'
...
3. [Language Consistency - Quoted content must be translated to the report language]
   ...
   - Maintain the original meaning when translating, ensure the expression is natural and fluent
...
4. [Faithfully present prediction results]
   ...
   - If information on certain aspects is insufficient, state it honestly
...
- Use **bold**, paragraph separators, quotes, lists to organize content, but do not use headings

[Correct Example]
```
This section analyzed the public opinion propagation dynamics of the event. Through in-depth analysis of simulation data, we found...
...
4. [Quote Format Specification - Must be a standalone paragraph]
   Quotes must be an independent paragraph with a blank line before and after, cannot be mixed into a paragraph:
...
3. Please use a mix of different tools, do not only use one type
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You have only called {tool_calls_count} tools, at least {min_tool_calls} calls required."
    "Please call more tools to retrieve more simulation data, then output Final Answer.{unused_hint}"
)
...
REACT_UNUSED_TOOLS_HINT = "\n\U0001F4A1 You have not yet used: {unused_list}. It is recommended to try different tools for multi-angle information"
...
class ReportAgent:
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempt when format 1 does not match, to avoid false matches in body text JSON
    ...
    def generate_report(
        ...
        try:
            ...
            # Phase 2: Generate section by section (save individually)
