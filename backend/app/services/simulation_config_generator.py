...
Uses LLM to auto-generate detailed simulation parameters based on requirements, documents, and graph info
...
class TimeSimulationConfig:
    ...
    # Off-peak hours (0-5am, almost no activity)
...
class SimulationConfigGenerator:
    ...
    auto-generating optimal simulation parameter configs
...
- User group is Chinese, must follow Beijing time daily schedule
- 0-5am almost no activity (activity coefficient 0.05)
- 6-8am gradually active (activity coefficient 0.4)
- 9am-6pm moderately active (activity coefficient 0.7)
- 7-10pm is peak time (activity coefficient 1.5)
...
- **Important**: Example values below are for reference only, adjust based on event nature and participant characteristics
  - E.g.: Student peak may be 9-11pm; media active all day; government agencies only during work hours
...
For example: official statements by Official/University types, news by MediaOutlet, student opinions by Student.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Track used agent indices per type to avoid reusing same agent
...
- **Schedule follows Chinese daily patterns**: almost no activity 0-5am, most active 7-10pm
