...
# Adding this prefix prevents Agent from calling tools and replies directly with text
    ...
    Request (JSON):
        {
            ...
            "graph_id": "swarm_xxxx",    // Optional, fetched from project if not provided
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task does not exist, but if simulation_id provided, check preparation
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    return the latest one if multiple (sorted by created_at)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // Optional: max simulation rounds, truncates long simulations
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it will stop and clear run logs
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running, call /stop to stop first, or use force=true to force restart"
                            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions shows content from both platforms for the latest round only
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // Optional, specify platform (twitter/reddit)
                                               // When not specified: dual-platform simulation interviews on both platforms
